  ...
  return (
    <>
      <div className={`h-9 rounded-lg transition-all duration-300  flex items-center mt-2
            ${menu === "watchlist" ? "bg-gradient-to-r from-yellow-200 to-yellow-400" : ""}
             ${isScale ? "w-36 justify-start px-3" : "w-9  justify-center"}`}>
        <Link
          to="/watchlist"
          className="flex cursor-pointer">
          <SVG.WatchList menu={menu} />
          <div className={`transition-all duration-300
                 ${menu === "watchlist" ? "text-black" : "text-white"}
                ${isScale ? "opacity-100" : "hidden"}`} >
            <span className={`font-normal ml-5`}>WatchList</span>
          </div>
        </Link>
      </div>
    </>
